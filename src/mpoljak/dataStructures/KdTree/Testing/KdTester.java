...
import mpoljak.data.test.Data2D;
...
public class KdTester<D extends ISame<D> & IKdComparable<D>, K extends Comparable<K>> {
    ...
    private static final int DEBUG_DUPLICATE_SEED = 1658875354;         // for testing: OVERALL
...
//        D nextData = generateDataInstance(valGen);
    ...
    /**
     * Test of k-d tree using all operations (which are generated by specified probabilities) and checking tree content
     * validity.
     * @param insProb probability of insertion operation into the k-d tree
     * @param searchProb probability of searching operation in k-d tree
     * @param delProb probability of deleting operation from k-d tree
     * @param dupProb probability of generating data with duplicate secondary key (from already inserted data)
     * @param operationsCount overall number of executed operations
     * @param logLevel the level of details displayed in console. From -1 to 3.
     * @param debug if constants for generator seeds are to be used
     * @return true, if all operations did what was expected. Else false.
     */
    public boolean doOverallTest(double insProb, double searchProb, double delProb, double dupProb, int operationsCount,
                                 int logLevel, boolean debug) {
        ...
        int duplicateSeed = debug ? DEBUG_DUPLICATE_SEED : genR.nextInt();
        Random dupGen = new Random(duplicateSeed);
        ...
        for (int op = 1; op <= operationsCount; op++) { // do n operations
            ...
            if (comp.compare(chosenOperation, insProb) < 1) {           //  chosenOperation <= insProb
                D nextData;
                if (comp.compare(dupGen.nextDouble(), dupProb) < 1 && !lObserved.isEmpty()) {
                    nextData = this.dataGenerator.copyConstruct(
                            lObserved.get( valGen.nextInt(lObserved.size()) ),
...
//        -----------------------------------------------------
        if (chosenOperation == OVERALL_TEST) {
            if (!debug) {
                for (int p = 1; p <= 51; p+=10) {
                    for (int i = 10; i < 100; i += 10) {
                        for (int s = 10; s < (100 - i); s += 10) {
                            int d = (100 - i - s);
                            System.out.println(String.format("\ninsert=%d%% , search=%d%% , delete=%d%%, duplicates=%d%%", i, s, d, p));
                            boolean testOk = ot.doOverallTest(i / 100.0, s / 100.0, d / 100.0, p / 100.0, 20000, 0, false);
                ...
                allOk = ot.doOverallTest(0.4, 0.2, 0.4, 0.35,100000, 0, false);
            }
        }
//        -----------------------------------------------------
